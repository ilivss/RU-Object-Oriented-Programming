...
public class Term {
	...
	public void plus(Term t) {
        ...
        /**
	 * For subtracting two terms with equal exponents
	 *
	 * @param t the term added to this
	 * @require exponent == t.exponent
	 */
	...
	public String toString() {
		if (exponent == 0) {
			return String.format("%s", removetailzeros(coefficient));
		} else if (exponent == 1) {
			return String.format("%sx", removetailzeros(coefficient));
		} else {
			return String.format("%sx^%d", removetailzeros(coefficient), exponent);
		}
	}
        
        private static String removetailzeros (double d) {
            if(d == (long) d)
